# Script that generates LaTex tables for the top-n FL techniques and the comparison of the
# best FL technique with all others, considering
# different scoring schemes, aggregation functions, and score vs. rank values.
#
# usage: Rscript analyze_best.R <data_file> <out_dir> [n]
#
...
if (length(args)<2) {
    stop("usage: Rscript analyze_best.R <data_file> <out_dir> [n -- default is 25]")
}
        ...
        for (agg_column in scoring_metrics) {
            ...
            for (agg_function in agg_functions) {
                ...
                cat(
                    ...
                    formatTechnique(best_technique_df), " & ",
