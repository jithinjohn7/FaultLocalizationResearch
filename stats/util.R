...
getTechniqueMacro <- function(technique) {
  return(
         # SBFL
         ifelse(technique=="ochiai",     "Ochiai",
         ifelse(technique=="barinel",    "Barinel",
         ...
         ifelse(technique=="failover",       "\\failover",
         ifelse(technique=="susp-maxing",    "\\suspmaxing",
         ifelse(technique=="susp-averaging", "\\suspaveraging",
         # MRSBFL (mutation coverage)
         ifelse(technique=="mbfl-coverage-only",      "\\mrsbfl",
         # MRSBFL-hybrid (mutation coverage + sbfl)
         ifelse(technique=="mrsbfl-failover",         "\\mrsbflfailover",
         ifelse(technique=="mrsbfl-susp-maxing",      "\\mrsbflsuspmaxing",
         ifelse(technique=="mrsbfl-susp-averaging",   "\\mrsbflsuspaveraging",
    "NA")))))))))))))))))
}
...
rankTopN <- function(df) {
    agg <- aggregate(ScoreAbs ~ ScoringScheme + Real + FLT, data=df, FUN=function(x) averageTopN(x))
    agg <- agg[with(agg, order(Real, ScoringScheme, -ScoreAbs)),]
