# Script that plots the distributions of the exam scores and absolute scores of
# the new (hyrbid) techniques. 
#
# usage: Rscript newTechniques.R <data_file_real_exploration> <out_dir>
#

# Read file name of the data file and the output directory
args <- commandArgs(trailingOnly = TRUE)
if (length(args)!=2) {
    stop("usage: Rscript newTechniques.R <data_file_real_exploration> <out_dir>")
}
data_file <- args[1]
out_dir <- args[2]
...
# Compare the new (hybrid) techniques with the best sbfl and mbfl techniques
...
df_mrsbfl <- subset(df, !df$ScoringScheme=="mean" & 
           (df$FLT %in% c("Metallaxis", "DStar", "MRSBFL-hybrid-avg", "MCBFL-hybrid-avg")))
df_mrsbfl$FLT <- factor(df_mrsbfl$FLT, levels=c("MCBFL-hybrid-avg",
                                                "MRSBFL-hybrid-avg",
                                                "Metallaxis",
                                                "DStar"))
levels(df_mrsbfl$FLT)[levels(df_mrsbfl$FLT)=="MCBFL-hybrid-avg"] <- "MKill+MCov+SCov"
levels(df_mrsbfl$FLT)[levels(df_mrsbfl$FLT)=="MRSBFL-hybrid-avg"] <- "MCov+SCov"
...
line  <- scale_linetype_manual(name="FL technique: ", values=c("solid", "solid", "twodash", "dashed"))
color <- scale_color_manual(name="FL technique: ", values=c("blue", "red", "gray50", "gray30"))
# Plot the distribution of exam scores (log scale) -- MRSBFL techniques, all projects
pdf(file=paste(out_dir, "mrsbfl_distributions_ratio.pdf", sep="/"), pointsize=20, family="serif", width=15, height=11)
ggplot(df_mrsbfl, aes(x=ScoreWRTLoadedClasses, color=FLT, linetype=FLT)) + geom_line(stat="density", size=1.5) +
line + color + theme_bw() +
facet_grid(Scheme~.) + labs(x="EXAM score (log scale)", y="Density", color="FL technique: ") +
theme +  scale_x_log10(breaks = c(0.0001,0.001,0.01,0.1,1))
dev.off()

# Plot the distribution of absolute scores (log scale) -- MRSBFL techniques, all projects
pdf(file=paste(out_dir, "mrsbfl_distributions_abs.pdf", sep="/"), pointsize=20, family="serif", width=15, height=11)
ggplot(df_mrsbfl, aes(x=ScoreAbs, color=FLT, linetype=FLT)) + geom_line(stat="density", size=1.5) +
line + color + theme_bw() +
facet_grid(Scheme~.) + labs(x="Absolute score (log scale)", y="Density", color="FL technique: ") +
theme + scale_x_log10(breaks = c(1,10,100,1000,10000))
dev.off()
